...
export function ActionsCell({ user, lang, roles }: ActionsCellProps) {
  ...
  return (
    <>
      <div className="flex items-center gap-1">
        <Link href={`/${lang}/admin/users/${user.id}`} className={btnClass} aria-label="View user">
          <ViewIcon />
        </Link>
        <button onClick={() => setShowEdit(true)} className={btnClass} aria-label="Edit user">
          <EditIcon />
        </button>
        <button onClick={() => setShowDelete(true)} className={btnClass} aria-label="Delete user">
          <DeleteIcon />
        </button>
      </div>

      {showEdit && (
        <EditUserModal user={user} roles={roles} onClose={() => setShowEdit(false)} />
      )}
      {showDelete && (
