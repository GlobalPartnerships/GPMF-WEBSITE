...
export function ActionsCell({ report, lang }: ActionsCellProps) {
  ...
  return (
    <>
      <div className="flex items-center gap-1">
        <button className={btnClass} aria-label="View report">
          <ViewIcon />
        </button>
        <button onClick={() => setShowEdit(true)} className={btnClass} aria-label="Edit report">
          <EditIcon />
        </button>
        <button onClick={() => setShowDelete(true)} className={btnClass} aria-label="Delete report">
          <DeleteIcon />
        </button>
      </div>

      {showEdit && (
        <EditReportModal report={report} onClose={() => setShowEdit(false)} />
      )}
      {showDelete && (
        <DeleteReportModal report={report} onClose={() => setShowDelete(false)} />
      )}
    </>
  );
}
