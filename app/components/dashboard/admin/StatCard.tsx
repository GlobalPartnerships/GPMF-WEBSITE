  ...
  return (
    <div className={`${dashStyles.card} whisper-shadow bg-white rounded-sm p-6`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={icon === "earnings" ? "text-burgundy/60" : "text-surface-variant/40"}>
          <Icon />
        </span>
      </div>
      <div className="flex items-end gap-2.5">
        <span className="font-serif text-[30px] font-bold leading-tight tracking-[-0.02em]">
          {value}
        </span>
        <span
          className={`text-[11px] tracking-[0.04em] uppercase font-semibold flex items-center mb-1.5 ${
            isUp ? "text-[#2e7d52]" : "text-[#ba1a1a]"
          }`}
        >
          {isUp ? <TrendUpIcon /> : <TrendDownIcon />} {trend.value}
        </span>
      </div>
      <p className="text-surface-variant text-[13px] mt-1.5">{subtitle}</p>
    </div>
  );
}
